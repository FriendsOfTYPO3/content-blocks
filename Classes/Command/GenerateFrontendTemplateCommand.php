...
#[Autoconfigure(tags: [
    [
        ...
        'command' => 'content-blocks:generate:frontend',
        'description' => 'Generate a frontend template for a Content Block',
        'schedulable' => false,
    ],
])]
class GenerateFrontendTemplateCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this->addArgument(
            ...
            'The Content Block name to generate the frontend template for (e.g. "vendor/name").'
        );
        $this->addOption(
            'force',
            'f',
            InputOption::VALUE_NONE,
            'Override existing frontend template.'
        );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($contentBlock->getContentType() !== ContentType::CONTENT_ELEMENT) {
            $output->writeln('<error>Content Block "' . $contentBlockName . '" is not a Content Element.</error>');
            return Command::INVALID;
        }
        ...
        if (file_exists($filePath) && !$input->getOption('force')) {
            $output->writeln('<error>Frontend template for Content Block "' . $contentBlockName . '" already exists at "' . $filePath . '". Use --force to override.</error>');
            return Command::INVALID;
        }

        $result = $this->htmlTemplateCodeGenerator->generateFrontendTemplate($contentBlock, $this->tableDefinitionCollection);
